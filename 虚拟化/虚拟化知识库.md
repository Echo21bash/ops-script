...
# 基础清理（清理已停止的容器、悬空镜像、未使用的网络）
docker system prune -f
# 深度清理（清理所有未被使用的镜像、容器、卷以及构建缓存）
...
```shell
#找出体积最大的 Overlay 目录
du -sh /var/lib/docker/overlay2/* | sort -rh | head -n 5
#将 Overlay 文件夹 ID 反查对应容器
docker ps -q | xargs docker inspect --format '{{.Name}} -> {{.GraphDriver.Data.MergedDir}}' | grep <文件夹ID>
#进入容器排查
# 进入该 overlay2 目录下的 diff 目录（存放容器运行期间产生的所有增量数据）
cd /var/lib/docker/overlay2/b1d4849c7cdf787728748179c9a79fb560955a6b3629d8ddf66f1f28a477e116/diff
# 查找当前目录下体积超过 100M 的文件并按大小排序
...
* 强制删除Terminating的命令空间

```shell
# 清除某个命名空间的finalizers
kubectl patch namespace --patch '{"metadata": {"finalizers": null}}'
# 获取所有状态为Terminating的ns，然后移除ns中的finalizers
kubectl get ns | grep Terminating | awk '{print $1}' | xargs -I {} kubectl patch namespace {} -p '{"metadata":{"finalizers":[]}}' --type=merge'{"metadata":{"finalizers":[]}}' --type=merge
```
...
> 需要安装/Oracle_VM_VirtualBox_Extension_Pack扩展包，并启动usb3.0
...
> 安装 macOS unlocker 补丁,在 https://github.com/DrDonk/unlocker/releases 下载补丁包，使用unlock工具破解mac支持，破解后使用check检查如下图
...
或者使用第三方引导如OpenCore或者Clover 模拟完整的mac信息，使用苹果系统完整的功能，参考地址https://heipg.cn/tutorial/macserial-and-iservice-opencore.html

### 客户机操作系统禁用CPU

> 关闭虚拟机 找到 虚拟机所在目录macOS 12-0.vmx 配置文件添加如下内容，一般是AMD处理器会碰到
